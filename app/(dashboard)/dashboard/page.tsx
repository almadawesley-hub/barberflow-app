      ...
      {summary && (
        <>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <MetricCard icon={DollarSign} value={`R$ ${fmt(summary.totalMes)}`} label="Faturamento (mês)" />
            <MetricCard icon={Ticket} value={`R$ ${fmt(Math.round(summary.ticketMedio))}`} label="Ticket médio" />
            <MetricCard icon={Users} value={String(summary.atendConcluidos)} label="Atendimentos hoje" sub={`${summary.atendAgendados} agendados`} />
            <MetricCard icon={PackageX} value={String(summary.estoqueBaixo)} label="Estoque baixo" sub={`${summary.esgotados} esgotados`} />
          </div>
            ...
            {summary.ranking.map((b, i) => (
              <div key={b.id} className="flex items-center gap-2.5 py-2 border-b border-ink-line last:border-0">
                ...
                <div
                  ...
                  style={{ background: b.colorHex ?? "var(--color-muted)", color: "var(--color-ink)" }}
                >
                  {initials(b.name)}
                </div>
...
function MetricCard({
  ...
  return (
    <div className="bg-ink-soft border border-ink-line rounded-2xl p-4">
      <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-2.5 bg-ink-line/60">
        <Icon size={16} className="text-ivory" />
      </div>
      <div className="font-display text-lg font-semibold text-ivory">{value}</div>
...
function SectionCard({ icon: Icon, children }: { icon: any; children: React.ReactNode }) {
  return (
    <div className="flex gap-3 bg-ink-soft border border-ink-line rounded-2xl p-4 mb-3">
      <div className="w-9 h-9 rounded-lg bg-ink-line/60 flex items-center justify-center flex-shrink-0">
        <Icon size={16} className="text-ivory" />
      </div>
